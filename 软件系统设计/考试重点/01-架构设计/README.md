...
- 重点要考的 6 类架构：主机/终端、C/S、三层/分层、SOA、微服务、事件驱动/云原生。
- “9种架构模式”是更宽的复习背景，期末重点要能把 6 类架构讲清楚。
- 架构迁移不是“旧模式错了”，而是系统规模、复杂度、组织成本、协作方式变化后，旧模式增长出来的矛盾已经不再适合当前主要问题。
- 复习时每种架构都要回答：系统边界在哪里、职责如何划分、组件之间如何协作。
...
#### 2.16.9 Event-driven / Cloud-native 事件驱动与云原生

核心思想：

- 事件驱动架构通过事件生产者、事件通道和事件消费者解耦系统组件。
- 云原生强调容器化、自动化部署、弹性伸缩、可观测性和面向平台的运行方式。

解决的问题：

- 同步调用链过长、服务之间耦合强。
- 高并发场景下需要削峰填谷。
- 系统需要弹性伸缩、快速部署、故障隔离和自动运维。
...
- 事件驱动/云原生重点不在背概念，而在解释为什么系统从微服务进一步走向异步事件、容器化部署和平台化运维。
- 如果题目给出高并发、异步通知、订单状态流转、日志分析、消息推送等场景，可以考虑事件驱动。
- 如果题目强调部署运维、弹性伸缩、容器化、自动恢复，可以联系云原生。
...
迁移不是因为旧架构“错误”，而是旧架构在新规模、新复杂度、新组织协作方式下暴露出新瓶颈：

- 主机/终端：集中计算和集中数据带来主机瓶颈、交互能力弱、扩展困难。
- C/S：客户端安装、升级、维护成本高，版本兼容和部署管理困难。
- 三层/分层：改善职责分离，但在企业级系统集成、跨系统复用和异构互操作上仍不足。
- SOA：提升服务复用和互操作，但 ESB/治理复杂度、性能瓶颈和集中式中介问题明显。
- 微服务：提升独立部署和扩缩能力，但带来分布式一致性、调用链、运维监控复杂度。
- 事件驱动/云原生：进一步提升弹性、异步解耦和运维自动化，但牺牲可追踪性、调试简单性和一致性。

#### 2.16.11 质量属性取舍总览

| 架构 | 优先保障 | 可能牺牲 |
|---|---|---|
| 主机/终端 | 一致性、安全性、可控性、集中管理 | 交互体验、灵活性、可伸缩性 |
| C/S | 交互体验、响应速度、部分性能 | 客户端维护、部署升级、兼容性 |
| 三层/分层 | 可修改性、可维护性、职责分离 | 性能、层间调用开销 |
| SOA | 互操作性、服务复用、系统集成 | 性能、治理复杂度、安全攻击面 |
| 微服务 | 可部署性、可伸缩性、团队自治、故障隔离 | 数据一致性、运维复杂度、调用链追踪 |
| 事件驱动/云原生 | 弹性伸缩、异步解耦、容器化运维 | 可追踪性、调试难度、最终一致性 |

#### 2.16.12 微服务设计题新增提醒

新增截图明确提醒：

- 可能出现 20 分左右的微服务架构分析设计题。
- 微服务“基础概念”不是主要考点，重点是设计核心模式和场景应用。
- 服务拆分要按业务能力划分，而不是按技术层随便切。
- 重构策略不作为重点。
- 六边形架构图要会画：领域核心在中心，外部通过端口/适配器接入 UI、数据库、消息、外部服务。
- 组合考法：服务拆分 + 核心模式 + 部署运维；其中部署运维要特别注意容器化部署。
- ADD 中“实例化元素并定义接口”的图表/文档表达要掌握。
